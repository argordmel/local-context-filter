...
- `--grep`, `--ls`, and `--find` auto-skip `.git`, `node_modules`, `dist`,
  `build`, `.venv`, `__pycache__`, `.next`, `coverage` (plus any project
  excludes above); `--grep` and `--find` also cap at 500 matches.
